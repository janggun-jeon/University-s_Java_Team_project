...
class ProcessData{//기본 데이터 속성. 지연시간과 주문을 가짐.
    int relatedtime;
    Order order;
}
public class Process{

    private static Queue<ProcessData> tem = new LinkedList<ProcessData>();//처리가 지연된 주문들을 넣기 위해 임시 큐 구현
    
    ProcessData element;//기본 데이터 속성 

    Process(Order order){//객체 생성과 동시에 객체의 지연시간 계산, 지연시간 계산 방법: 지연시간 =  기본 지연시간 1+ 수량/3 +무게/5
        element=new ProcessData();
...
class Classification{//분류작업 클래스
    
    private static ArrayList<ProcessData> ClassifyingOrder = new ArrayList<ProcessData>(4);//arraylist에 현재 분류작업중인 주문들을 넣는다.
    private static ArrayList<Order> ReturnOrder = new ArrayList<Order>();

    public static boolean AddOrder(ProcessData delayorder){//리스트의 용량이 초과되지 않는다면 데이터를 넣고 참값을 반환한다.
